...
// Guarda SOLO lo que cambió — no todos los 5000 pacientes cada vez
// ========================================

const DB_MAIN = () => db.collection('clinicaData-dev').doc('main');
const DB_PACIENTES = () => db.collection('clinicaData-dev').doc('main').collection('pacientes');
...
// Guardar SOLO datos principales (facturas, citas, gastos, etc.) — instantáneo
async function saveMainData() {
    await DB_MAIN().set(getMainPayload());
}

// Guardar UN paciente individual — muy rápido (~200ms)
async function savePacienteDoc(paciente) {
    await DB_PACIENTES().doc(paciente.id).set(paciente);
}

// Eliminar UN paciente individual — muy rápido
async function deletePacienteDoc(pacienteId) {
    await DB_PACIENTES().doc(pacienteId).delete();
}

// Guardar TODOS los pacientes en batch — solo para importación masiva
async function saveAllPacientes() {
    ...
    for (let i = 0; i < appData.pacientes.length; i += BATCH_SIZE) {
        ...
        lote.forEach(p => batch.set(DB_PACIENTES().doc(p.id), p));
        await batch.commit();
        console.log(`✅ Pacientes: ${Math.min(i + BATCH_SIZE, appData.pacientes.length)}/${appData.pacientes.length}`);
    }
}

// saveData — wrapper compatible con código existente
// Acepta opciones para ser selectivo en qué guarda
async function saveData(opciones = {}) {
    const { paciente = null, deletePacienteId = null, soloMain = false } = opciones;
    try {
        if (deletePacienteId) {
            // Eliminar paciente: solo borrar el doc + actualizar main
            await Promise.all([saveMainData(), deletePacienteDoc(deletePacienteId)]);
        } else if (paciente) {
            // Crear/editar paciente: solo guardar ese doc + actualizar main
            await Promise.all([saveMainData(), savePacienteDoc(paciente)]);
        } else if (soloMain) {
            // Operaciones que no tocan pacientes (facturas, citas, gastos, etc.)
            await saveMainData();
        } else {
            // Full save — solo importación masiva
            await saveMainData();
            await saveAllPacientes();
        }
        updateLocalCache();
        showSyncIndicator();
    } catch (error) {
            ...
            <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
                <thead>
                    <tr style="background: #f5f5f5;">
                        <th style="padding: 8px; text-align: left; color: #002366;">Descripción</th>
...
function renderTabResumen(paciente) {
    ...
    document.getElementById('tabResumen').innerHTML = `
        ...
        <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); margin-bottom: 24px;">
            <h3 style="font-size: 16px; color: #002366; margin-bottom: 16px; font-weight: 700;">Información Personal</h3>
            ...
            if (recetas.length > 0) {
                ...
                return `
                    <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                        <h3 style="font-size: 16px; color: #002366; margin-bottom: 12px; font-weight: 700;">💊 Última Receta</h3>
...
function renderTabHistorial(paciente) {
    ...
    document.getElementById('tabHistorial').innerHTML = `
        <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); margin-bottom: 20px;">
            <h3 style="font-size: 16px; color: #002366; margin-bottom: 16px; font-weight: 700;">📋 Procedimientos</h3>
        ...
        <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); margin-bottom: 20px;">
            <h3 style="font-size: 16px; color: #002366; margin-bottom: 16px; font-weight: 700;">🔬 Laboratorio</h3>
        ...
        <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
            <h3 style="font-size: 16px; color: #002366; margin-bottom: 16px; font-weight: 700;">📅 Citas</h3>
    ...
    document.getElementById('tabDocumentos').innerHTML = `
        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px;">
            ...
            <div style="background: white; border-radius: 12px; padding: 24px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); text-align: center;">
                ...
                <h3 style="font-size: 16px; color: #002366; margin-bottom: 8px; font-weight: 700;">Placas Radiográficas</h3>
    ...
    galeriaContainer.innerHTML = placasOrdenadas.map(placa => {
        ...
        <div class="placa-card" style="background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1); transition: transform 0.2s;" onmouseover="this.style.transform='translateY(-4px)'" onmouseout="this.style.transform='translateY(0)'">
            ...
            <div style="padding: 15px;">
                <div style="font-size: 14px; font-weight: 600; color: #002366; margin-bottom: 8px;">${placa.tipo || 'Radiografía'}</div>
                ${placa.subidoPor ? `
...
async function guardarPlaca() {
    ...
    try {
        ...
        await saveData({ paciente: currentPacienteGaleria });
...
async function guardarReceta() {
    ...
    await saveData({ paciente: currentPacienteRecetas });
